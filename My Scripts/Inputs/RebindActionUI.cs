...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private bool SwapResetBindings(InputAction action, int bindingIndex)
        {
            ...
            for (int i = 0; i < action.actionMap.bindings.Count; ++i)
            {
                ...
                if (binding.effectivePath == newBinding.path)
                {
                    ...
                    //Debug.Log($"Duplicate bind index of action map: {i}");

                    // Swap the two actions.
                    //action.actionMap.FindAction(binding.action).ApplyBindingOverride(i, newBinding.overridePath);
                    //action.RemoveBindingOverride(bindingIndex);
                    InputAction actionToSwap = action.actionMap.FindAction(binding.action);
                    Utilities.ReadOnlyArray<InputBinding> actionToSwapBindings = actionToSwap.bindings;

                    for (int j = 0; j < actionToSwapBindings.Count; j++)
                    {
                        if (binding.effectivePath == actionToSwapBindings[j].effectivePath)
                        {
                            //Debug.Log($"Duplicate bind index of action to swap: {j}");

                            actionToSwap.ApplyBindingOverride(j, newBinding.overridePath);
                            break;
                        }
                    }
                    action.RemoveBindingOverride(bindingIndex);
        ...
        public void StartInteractiveRebind()
        {
            ...
            {
                originalBinding = action.bindings[bindingIndex];
                PerformInteractiveRebind(action, bindingIndex);
            }
        }

        void SwapBindings(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            // Cache a reference to the current binding.
            InputBinding newBinding = action.bindings[bindingIndex];
            ...
            for (int i = 0; i < action.actionMap.bindings.Count; ++i)
            {
                InputBinding binding = action.actionMap.bindings[i]; // tää käy kaikki läpi
                ...
                if (binding.effectivePath == newBinding.effectivePath)
                {
                    ...
                    for (int j = 0; j < actionToSwapBindings.Count; j++)
                    {
                        if (binding.effectivePath == actionToSwapBindings[j].effectivePath)
                        {
                            ...
                            //{
                            //    actionToSwap.ApplyBindingOverride(j, originalBinding.effectivePath);
                            //}
                            //else actionToSwap.ApplyBindingOverride(j, originalBinding.effectivePath);
                            actionToSwap.ApplyBindingOverride(j, originalBinding.effectivePath);
                        }
                    }
                }
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                ...
                .WithControlsExcluding("<Pointer>/scroll")
                ...
                .OnComplete(
                    operation =>
                    {
                        ...
                        {
                            if (!allCompositeParts) SwapBindings(action, bindingIndex, allCompositeParts);
                            else
                            {
                                action.RemoveBindingOverride(bindingIndex);
                                CleanUp();
        ...
        {
            InputBinding newBinding = action.bindings[bindingIndex]; //se mitä painetaan
